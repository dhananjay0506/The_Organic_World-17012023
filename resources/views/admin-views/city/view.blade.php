                ...
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form action="{{route('admin.city.store')}}" method="post">
                        ...
                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                            ...
                            <label for="name">{{ \App\CPU\translate('City')}} {{ \App\CPU\translate('Name')}}
                                    ({{strtoupper($lang)}})</label>
                            <input type="text" name="name[]" class="form-control" id="name"
                                   placeholder="{{\App\CPU\translate('Enter_City_Name')}}" {{$lang == $default_lang? 'required':''}}>
                        </div>
                ...
                <div class="card-header">
                    <div class="row flex-between justify-content-between align-items-center flex-grow-1">
                            <div class="col-12 col-md-7">
                                <h5>{{ \App\CPU\translate('City')}} {{ \App\CPU\translate('Table')}} <span style="color: red;">({{ $citys->total() }})</span></h5>
                            </div>
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-merge input-group-flush">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            placeholder="{{\App\CPU\translate('Search_by_City_Name')}}" aria-label="Search orders" value="{{ $search }}" required>
                    ...
                    <div class="table-responsive">
                        <table id="datatable"
                            ...
                            @foreach($citys as $key=>$City)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td style="width: 40%;text-align:center;">{{$City['name']}}</td>
