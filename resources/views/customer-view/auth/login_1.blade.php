...
@push('css_or_js')
    ...
    <style>
        ...
        .nav-pills  .bg-custome-warming.active{
                ...
                <div class="card border-0 box-shadow">
                    <div class="card-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade" id="pills-mobile" role="tabpanel" aria-labelledby="pills-mobile-tab">
                                    ...
                                    <div class="form-group">
                                        <label for="si-mobile">{{ \App\CPU\translate('phone') }}</label>
                                        <input class="form-control mobile_input" type="number" name="user_id" id="si-mobile"
                                            style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                                            value="{{ old('user_id') }}"
                                            placeholder="{{ \App\CPU\translate('Enter_phone_number') }}"
                                            required>
                                    ...
                                    <div class="form-group otp_div d-none">
                                        <label for="si-otp">{{ \App\CPU\translate('OTP') }}</label>
                                        <input class="form-control otp_input" type="number"  id="si-otp"
                                            style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                                            value="{{ old('user_id') }}"
                                            placeholder="{{ \App\CPU\translate('Enter_otp') }}"
                                            >
                                            {{-- required --}}
                                        <div class="invalid-feedback">
                            ...
                            @endif --}}
                            <button class="btn btn-primary btn-block otp_button"
                            >{{ \App\CPU\translate('send_otp') }}</button>
                                    <button class="btn btn-primary btn-block sign_in_button d-none"
                                        >{{ \App\CPU\translate('sign_in') }}</button>
                               
                            </div>

                            {{--  --}}
                            <div class="tab-pane fade show active" id="pills-email_login" role="tabpanel" aria-labelledby="pills-email_login-tab">
                                <form class="needs-validation mt-2" autocomplete="off"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="email" name="user_id" id="si-email"
                                            style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                                            value="{{ old('user_id') }}"
                                            placeholder="{{ \App\CPU\translate('Enter_email_address') }}"
                                            required>
                                        <div class="invalid-feedback">
                            ...
                            <ul class="nav nav-pills my-2" id="pills-tab" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="bg-custome-warming nav-link btn-block active" id="pills-email_login-tab" data-toggle="pill" data-target="#pills-email_login" type="button" role="tab" aria-controls="pills-email_login" aria-selected="false">{{ \App\CPU\translate('login_with_email') }}</button>
                                  </li> 
                                <li class="nav-item" role="presentation">
                                  <button class="bg-custome-warming nav-link btn-block" id="pills-mobile-tab" data-toggle="pill" data-target="#pills-mobile" type="button" role="tab" aria-controls="pills-mobile" aria-selected="true">{{ \App\CPU\translate('login_with_mobile') }}</button>
                                </li>
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-12 flex-between row p-0"
...
$( ".otp_button" ).click(function(res) {
    var mobile = $( ".mobile_input" ).val();

    $.get({
        url: '{{route('login_otp-verification')}}',
        dataType: 'json',
        data: {
            identity: mobile
        },
        ...
        success: function (data) {
            ...
            $('#loading').hide();
            $('.sign_in_button').removeClass( "d-none" );
...
$( ".sign_in_button" ).click(function() {
    var opt = $( ".otp_input" ).val();

    $.post({
        url: '{{route('login_otp_verification_submit')}}',
        dataType: 'json',
        headers:
