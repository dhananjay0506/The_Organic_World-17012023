...
@push('css_or_js')
    <script
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD-SD7cNmH2t9Ebn3U492VlK5uu4u_RUGY&libraries=drawing,places">
    </script>
...
@section('content')
    <div class="content container-fluid main-card {{ Session::get('direction') }}">
        ...
        <form class="user" action="{{ route('seller.apply') }}" method="post" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="parent_id" value="{{$seller->id}}">
            <div class="card">
                <div class="card-body">
                    ...
                    <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2 border-bottom pb-3 mb-4 pl-4">
                        <img src="{{ asset('/public/assets/back-end/img/seller-information.png') }}" class="mb-1"
                            alt="">
                        {{ \App\CPU\translate('employee_information') }}
                    </h5>
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="form-group">
                                <div class="title-color mb-2 d-flex gap-1 align-items-center">Seller Image <span
                                        class="text-info">(Ratio 1:1)</span></div>
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-lg-6 form-group">
                            <label for="shop_name"
                                class="title-color d-flex gap-1 align-items-center">{{ \App\CPU\translate('Store') }}/{{ \App\CPU\translate('Warehouse_name') }}</label>
                            <input type="text" class="form-control form-control-user" id="shop_name" name="shop_name"
                                placeholder="{{ \App\CPU\translate('Ex: Jhone') }}"
                                value="{{ old('shop_name') }}"required>
                        </div>
                        <div class="col-lg-6 form-group">
                            <label for="pincode"
                                class="title-color d-flex gap-1 align-items-center">{{ \App\CPU\translate('select_pin_code') }}</label>
                            <select name="pincode[]" class="form-control" multiple multiselect-search="true"
                                multiselect-select-all="true" multiselect-max-items="50">
                        ...
                        <div class="col-lg-6 form-group">
                            <label for="shop_address"
                                class="title-color d-flex gap-1 align-items-center">{{ \App\CPU\translate('Store') }}/{{ \App\CPU\translate('Warehouse_address') }}</label>
                            ...
                            <input type="text" id="address-input" name="shop_address"
                        ...
                        <div class="col-sm-6 form-group">
                            <label for="geo_boundaries" class="title-color d-flex gap-1 align-items-center">Shop Geo
                                Boundaries</label>
                            <div id="map_canvas" style="width:450px; height:450px;"></div>
                            <form method="post" accept-charset="utf-8" id="map_form">
                                <input type="hidden" class="form-control" name="geo_boundry" value=""
                                    id="vertices" />
                        ...
                        <div class="form-group">
                            <input class="form-control" type="hidden" name="lat" id="address-latitude"
                                onchange="Lat_long()" />


                        </div>
                        <div class="form-group">
                            <input class="form-control" type="hidden" name="longt" id="address-longitude"
                                onchange="Lat_long()" />
                        ...
                        <div class="mt-4">
                            ...
                            <div class="custom-file">
                                <input type="file" name="banner" id="BannerUpload" class="custom-file-input"
                                    accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                <label class="custom-file-label" for="BannerUpload">{{ \App\CPU\translate('Upload') }}
...
@push('script')
    ...
    <script>
        ...
        function Lat_long() {
            alert("OOPS");
            var lat_v = document.getElementById('address-latitude').getAttribute("value") && document.getElementById(
                    'address-latitude').getAttribute("value") != '' ? document.getElementById('address-latitude').value() :
                16.691307;
            var long_v = document.getElementById('address-longitude').getAttribute("value") && document.getElementById(
                    'address-longitude').getAttribute("value") != '' ? document.getElementById('address-longitude')
            .value() : 74.244865;
            console.log(lat_v);
            console.log(long_v);
