...
class ProductController extends Controller
{
    public function add_new()
    {
        ...
        $br         = Brand::orderBY('name', 'ASC')->get();
          ...
          //if the product addede by subseller or employee of seller
          $seller_parent_id   = Seller::where('id', $seller_id)->first();

         if($seller_parent_id->parent_id > 0){
            
             $shop        = Shop::where('seller_id', $seller_parent_id->parent_id)->first();
         }
         else{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'code'              => 'required|min:3|unique:products',  //digits_between:3,20 removed
            ...
            // 'code.digits_between'   => 'The code must be minimum 3 digits!',
        ...
        if($seller_parent_id->parent_id > 0){
            ...
            $product->user_id   = $seller_parent_id->parent_id;
        }
        else{
        ...
        $product->created_by    = auth('seller')->id();
        $product->updated_by    = auth('seller')->id(); 
    ...
    function list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $products = Product::where('added_by', 'seller')->whereIn('user_id',[auth('seller')->id(),auth('seller')->user()->parent_id])
    ...
    public function stock_limit_list(Request $request, $type)
    {
        ...
        $search = $request['search'];
        $pro = Product::where('added_by', 'seller')->whereIn('user_id',[auth('seller')->id(),auth('seller')->user()->parent_id])
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'discount'          => 'required|gt:-1',
            'shipping_cost'     => 'required|gt:-1',
            'code'              => 'required|min:1|unique:products,code,'.$product->id, //|digits_between:6,20
            'minimum_order_qty' => 'required|numeric|min:1',
        ], [
            'name.required'                 => 'Product name is required!',
            'category_id.required'          => 'category  is required!',
