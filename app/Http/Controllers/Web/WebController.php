...
class WebController extends Controller
{
    ...
    public function home(Request $request)
    {
        ...
        $home_categories->map(function ($data) use ($Shop_pincode) {
            $id = '"'.$data['id'].'"';
        ...
        //feature products finding based on selling
         $featured_products = Product::select('products.*','shop_pincodes.pincode as shop_pincode')
                                    ->join('shop_pincodes','shop_pincodes.shop_id','products.shop_id')
                                    ->where('shop_pincodes.pincode',$Shop_pincode)
                                    ->with(['reviews'])->active()
                                    ->where('featured', 1)
                                    ...
                                    ->get();
        //end
        if(isset($Shop_pincode) && !empty($Shop_pincode))
        {
            $featured_products = Product::with(['reviews'])
            ->active()
            ->where('featured', 1)
            ->withCount(['order_details'])->orderBy('order_details_count', 'DESC')
            ->join('shops', 'shops.seller_id', '=', 'products.user_id')
            ->join('shop_pincodes', 'shops.id', '=', 'shop_pincodes.shop_id')
            ->where('shop_pincodes.pincode', '=', $Shop_pincode)
        ...
        if(isset($Shop_pincode) && !empty($Shop_pincode))
        {
            $latest_products = Product::with(['reviews'])->active()
            ->join('shops', 'shops.seller_id', '=', 'products.user_id')
            ->join('shop_pincodes', 'shops.id', '=', 'shop_pincodes.shop_id')
            ->where('shop_pincodes.pincode', '=', $Shop_pincode)
            ->where('products.added_by','=','seller')
        ...
        if(isset($request->city) && $request->city > 0)
        {
            ...
            // ->where('products.status', 1)
            // ->where('deal_of_the_days.status', 1)
            // ->where('shops.city_id', '=', $request->city)
            // ->where('products.added_by','=','seller')
            // ->first();
        }
        ...
        $products = $porduct_data->get();
            ...
            foreach ($products as $product) {
                foreach (json_decode($product['category_ids'], true) as $category) {
                    if ($category['id'] ==26 && count($product_ids) < 11) {
        ...
        $porduct_data_snack = Product::active()->with(['reviews']);

        $products_snack = $porduct_data_snack->get();
        if(isset($request->city) && $request->city > 0)
        {
            // $products_snack = $porduct_data_snack
            // ->join('shops as sh', 'sh.seller_id', '=', 'products.user_id')
            // ->where('products.status', 1)
            // ->where('sh.city_id', '=', $request->city)
            // ->where('products.added_by','=','seller')
            // ->get();
        }
            $product_ids = [];
            foreach ($products_snack as $product) {
                foreach (json_decode($product['category_ids'], true) as $category) {
                    if ($category['id'] ==38 && count($product_ids) < 11) {
                        array_push($product_ids, $product['id']);
                    }
                }
            }

         $sanck_sweet = $porduct_data_snack->whereIn('products.id', $product_ids)->get();
         if(isset($request->city) && $request->city > 0)
         {
            //  $sanck_sweet = $porduct_data_snack
            //  ->join('shops as sh', 'sh.seller_id', '=', 'products.user_id')
            // ->where('products.status', 1)
            // ->where('sh.city_id', '=', $request->city)
            //  ->where('products.added_by','=','seller')
            ...
            // home Essential
         $porduct_data_home_essential = Product::active()->with(['reviews']);

         $products_home_essential = $porduct_data_home_essential->get();
         if(isset($request->city) && $request->city > 0)
         {
            //  $products_home_essential = $porduct_data_home_essential
            //  ->join('shops as sh', 'sh.seller_id', '=', 'products.user_id')
            // ->where('products.status', 1)
            // ->where('sh.city_id', '=', $request->city)
            //  ->where('products.added_by','=','seller')
            //  ->get();
             ...
             $product_ids = [];
             foreach ($products_home_essential as $product) {
                 foreach (json_decode($product['category_ids'], true) as $category) {
                     if ($category['id'] ==33 && count($product_ids) < 11) {
                         array_push($product_ids, $product['id']);
                     }
                 }
             }
          ...
          if(isset($request->city) && $request->city > 0)
         {
            // $home_essential = $porduct_data_home_essential
            // ->join('shops as sh', 'sh.seller_id', '=', 'products.user_id')
            // ->where('products.status', 1)
            // ->where('sh.city_id', '=', $request->city)
            //  ->where('products.added_by','=','seller')
...
// return $deal;
    ...
    public function checkout_details(Request $request)
    {
        ...
        if($cart_shipping == null){
            ...
            // $cart_shipping->;
            // $cart_shipping->free_shipping_status = 0;
        ...
        if (count($cart_group_ids) > 0) {
            // return "hii";
            return view('web-views.checkout-shipping',compact('cart_shipping'));
    ...
    public function checkout_payment()
    {
        ...
        if (session()->has('address_id') && count($cart_group_ids) > 0) {
            return view('web-views.checkout-payment',compact('cart_shipping'));
        }
    ...
    public function product($slug)
    {
        ...
        $product = Product::select('products.*','shop_pincodes.id as shop_pincodes_id','shop_pincodes.pincode')
                            ->active()
                            ->with(['reviews'])
                            ->join('shop_pincodes','shop_pincodes.shop_id','products.shop_id')
                            ->where('shop_pincodes.pincode',$Shop_pincode)
        ...
        if ($product != null) {
            ...
            $relatedProducts = Product::select('products.*','shop_pincodes.id as shop_pincodes_id','shop_pincodes.pincode')
                                        ->with(['reviews'])
                                        ->active()
    ...
    public function products(Request $request)
    {
        ...
        if ($request['data_from'] == 'category') {
            ...
            $query = $porduct_data->whereIn('products.id', $product_ids);
        }

        if ($request['data_from'] == 'brand') {
            $query = $porduct_data->where('products.brand_id', $request['id']);
        }
        ...
        if ($request['data_from'] == 'featured_deal') {
            ...
            $query = Product::with(['reviews'])
                            ...
                            ->where('shop_pincodes.pincode',$Shop_pincode)
                            ->active()
                            ->whereIn('products.id', $featured_deal_product_ids);
        }
        ...
        if ($request['data_from'] == 'discounted') {
            $query = Product::with(['reviews'])
                            ->join('shop_pincodes','shop_pincodes.shop_id','products.shop_id')
                            ->where('shop_pincodes.pincode',$Shop_pincode)->active()
                            ->where('discount', '!=', 0);
        }
