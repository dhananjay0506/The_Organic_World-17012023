...
class ProductManager
{
    ...
    public static function get_latest_products($limit = 10, $offset = 1,$pincode)
    {
        
        $paginator = Product::select('products.*','shop_pincodes.pincode')
    ...
    public static function get_featured_products($limit = 10, $offset = 1,$pincode)
    {
    ...
    public static function get_top_rated_products($limit = 10, $offset = 1,$pincode)
    {
        ...
        // }
        //change review to ratting
        $reviews = Product::select('products.*','shop_pincodes.pincode')
                            ->join('shop_pincodes','products.shop_id','shop_pincodes.shop_id')
                            ->where('pincode',$pincode)
                            ->with(['rating'])
                            ->active()
    ...
    public static function get_best_selling_products($limit = 10, $offset = 1,$pincode)
    {
    ...
    public static function search_products($pincode, $name, $limit = 10, $offset = 1)
    {
        /*$key = explode(' ', $name);*/
        $key = [base64_decode($name)];

        $paginator = Product::active()->with(['rating'])
                ->select('products.*','shop_pincodes.pincode')
                ->join('shop_pincodes','products.shop_id','shop_pincodes.shop_id')
                ->where(function($q) use($pincode){
                    $q->where('pincode',$pincode);
                })
                ->where(function ($q) use ($key, $pincode) {
    ...
    public static function get_discounted_product($limit = 10, $offset = 1,$pincode)
    {
        //change review to ratting
        $paginator = Product::with(['rating'])
                            ->select('products.*','shop_pincodes.pincode')
